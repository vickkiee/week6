...
# $ touch $HOME/week6_scripts/my_week6_assignment.sh && chmod 777 $HOME/week6_scripts/my_week6_assignment.sh
...
reportFile="$HOME/week6_scripts/NAME-reports-$(date +%s)-$(date +%T).txt"

# Always append all echos to the report file before the actual echo
      
echo "$numberLine: .." >> $reportFilene=1;
...
handleEchoAndReport "DONE WITH One ✅ ..."
handleEchoAndReport
handleEchoAndReport
sleep 2
...
if [[ -z $isQuite ]]; then
                echo $msg
        fik_stats=$(ifstat 1 1 | awk 'NR==3{printf "In: %s, Out: %s", $1, $2}')
...
# recipient="martin.mato@ingrydacademy.com" # Only use MR Matin Email when done testing don't spam him!
recipient="vicwumi2006@gmail.com"
